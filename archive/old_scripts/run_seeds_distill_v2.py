"""
RSR with Knowledge Distillation v2 - REFINED

Improvements over v1:
1. CONFIDENCE-WEIGHTED LOSS: Weight distillation by how close unseen word is to trained neighbors
2. SIMILARITY THRESHOLD: Only distill to words with cosine sim > threshold to some trained word
3. MORE DISTILLATION: 2 epochs, more batches
4. PROGRESSIVE DISTILLATION: First wave distills closest words, second wave expands

Theory: Words that are distributionally VERY similar to trained words are more likely
to have similar semantic relationships. Words that are only weakly similar might have
different semantics despite co-occurrence patterns.

Usage:
    python run_seeds_distill_v2.py
"""
...
# Stage 2: Distillation v2 (REFINED)
DISTILL_EPOCHS = 2  # More epochs
DISTILL_BATCH_SIZE = 512  # Larger batches
DISTILL_K_NEIGHBORS = 15  # More neighbors for smoother interpolation
DISTILL_N_TARGETS = 150  # More targets per batch
DISTILL_BATCHES_PER_EPOCH = 3000
DISTILL_LR = 5e-4  # Smaller LR for finer updates

# NEW: Confidence thresholds
DISTILL_SIM_THRESHOLD = 0.4  # Only distill words with max_sim > this to trained words
DISTILL_CONFIDENCE_TEMP = 3.0  # Temperature for confidence weighting
...
# ==============================================================================
# STAGE 2: Distillation v2 - CONFIDENCE-WEIGHTED
# ==============================================================================

@torch.no_grad()
def precompute_unseen_confidences(model, unseen_indices, trained_tensor):
    """
    For each unseen word, compute its max similarity to any trained word.
    This serves as a "confidence" score for distillation.
    
    Returns: dict mapping unseen_idx -> max_sim_to_trained
    """
    ...
    batch_size = 10000
    for i in range(0, len(unseen_indices), batch_size):
        batch_idx = unseen_indices[i:i+batch_size]
        unseen_tensor = torch.tensor(batch_idx, dtype=torch.long, device=DEVICE)
        unseen_emb = Wn[unseen_tensor]  # [batch, dim]
        
        # Max similarity to any trained word
        sims = unseen_emb @ trained_emb.T  # [batch, n_trained]
        max_sims = sims.max(dim=1).values  # [batch]
        
        for idx, max_sim in zip(batch_idx, max_sims.cpu().numpy()):
            confidences[idx] = float(max_sim)
    
    return confidences

def distillation_loss_v2(model, unseen_tensor, unseen_confidences, trained_tensor, 
                          target_tensor, k_neighbors, neighbor_temp=5.0, conf_temp=3.0):
    """
    CONFIDENCE-WEIGHTED distillation loss.
    
    Each unseen word's loss is weighted by how close it is to trained words.
    Words closer to trained words get stronger gradients.
    """
    ...
    conf_weights = torch.tensor(
        [unseen_confidences[int(idx)] for idx in unseen_tensor.cpu().numpy()],
        dtype=torch.float32, device=DEVICE
    )
    # Apply temperature and normalize
    conf_weights = F.softmax(conf_weights * conf_temp, dim=0) * batch_size  # Scale to sum = batch_size
    
    # Detached embeddings for neighbor/target computation
    with torch.no_grad():
        W_detached = W.detach()
        Wn = W_detached / (W_detached.norm(dim=1, keepdim=True) + 1e-8)
        
        unseen_emb_detached = Wn[unseen_tensor]
        trained_emb = Wn[trained_tensor]
        target_emb_detached = Wn[target_tensor]
        
        # Find neighbors
        sims_to_trained = unseen_emb_detached @ trained_emb.T
        topk_sims, topk_idx = torch.topk(sims_to_trained, k_neighbors, dim=1)
        neighbor_weights = F.softmax(topk_sims * neighbor_temp, dim=1)
        
        neighbor_global_idx = trained_tensor[topk_idx]
        neighbor_emb = Wn[neighbor_global_idx]
        
        # Neighbor -> target similarities
        neighbor_to_target = torch.bmm(
            neighbor_emb,
            target_emb_detached.unsqueeze(0).expand(batch_size, -1, -1).transpose(1, 2)
        )
        
        # Weighted pseudo-similarities
        pseudo_sims = (neighbor_weights.unsqueeze(2) * neighbor_to_target).sum(dim=1)
    
    # Actual similarities (WITH gradient on unseen only)
    unseen_emb_grad = W[unseen_tensor]
    ...
    # Per-sample MSE loss
    per_sample_loss = ((actual_sims - pseudo_sims) ** 2).mean(dim=1)  # [batch]
    
    # Weight by confidence
    weighted_loss = (per_sample_loss * conf_weights).mean()
    
    return weighted_loss

def train_stage2_distillation_v2(model, optimizer, trained_indices_list, vocab_size,
                                  batches_per_epoch, batch_size, k_neighbors, n_targets,
                                  sim_threshold, conf_temp):
    """
    Stage 2 v2: Confidence-weighted distillation with similarity threshold.
    """
    model.train()
    ...
    # Precompute confidences
    print(f"  Computing confidence scores for {len(unseen_indices):,} unseen words...")
    confidences = precompute_unseen_confidences(model, unseen_indices, trained_tensor)
    
    # Filter by threshold
    eligible_unseen = [idx for idx in unseen_indices if confidences[idx] >= sim_threshold]
    print(f"  Eligible for distillation (sim >= {sim_threshold}): {len(eligible_unseen):,} / {len(unseen_indices):,}")
    
    if len(eligible_unseen) == 0:
        print("  No eligible words - skipping distillation")
        return {"loss": 0.0, "n_eligible": 0}
    
    all_vocab_indices = list(range(1, vocab_size))
    
    total_loss = 0.0
    pbar = tqdm(range(batches_per_epoch), desc="Stage2-Distill-v2", leave=False)
    
    for b in pbar:
        # Sample batch of eligible unseen words
        batch_idx = random.sample(eligible_unseen, min(batch_size, len(eligible_unseen)))
        unseen_tensor = torch.tensor(batch_idx, dtype=torch.long, device=DEVICE)
        
        # Sample targets (mix of trained and random)
        ...
        loss = distillation_loss_v2(
            model, unseen_tensor, confidences, trained_tensor, target_tensor,
            k_neighbors, neighbor_temp=5.0, conf_temp=conf_temp
        )
    ...
    return {"loss": total_loss / batches_per_epoch, "n_eligible": len(eligible_unseen)}
...
def run_one_seed(seed: int, word2idx, idx2word, idx_counts, train_words, pairs_array,
    ...
    # =========================================================================
    # Train RSR with Distillation v2
    # =========================================================================
    ...
    # Stage 2: Distillation v2
    print(f"[seed={seed}] Training RSR (Stage 2: Confidence-Weighted Distillation)...")
    distill_opt = optim.Adam(rsr_model.parameters(), lr=DISTILL_LR)
    
    for ep in range(1, DISTILL_EPOCHS + 1):
        stats = train_stage2_distillation_v2(
            model=rsr_model,
            optimizer=distill_opt,
            trained_indices_list=trained_indices_list,
            vocab_size=vocab_size,
            batches_per_epoch=DISTILL_BATCHES_PER_EPOCH,
            batch_size=DISTILL_BATCH_SIZE,
            k_neighbors=DISTILL_K_NEIGHBORS,
            n_targets=DISTILL_N_TARGETS,
            sim_threshold=DISTILL_SIM_THRESHOLD,
            conf_temp=DISTILL_CONFIDENCE_TEMP,
        )
        print(f"  [rsr-stage2] epoch {ep}/{DISTILL_EPOCHS} | loss={stats['loss']:.4f} | eligible={stats['n_eligible']:,}")
...
def main():
    parser = argparse.ArgumentParser(description="RSR with Distillation v2")
    parser.add_argument("--start_seed", type=int, default=1)
    parser.add_argument("--end_seed", type=int, default=20)
    parser.add_argument("--output", type=str, default="results/seed_results_distill_v2.xlsx")
    args = parser.parse_args()
    
    print(f"Device: {DEVICE}")
    print(f"Running seeds {args.start_seed} to {args.end_seed}")
    print(f"Output: {args.output}")
    print(f"\n*** RSR WITH DISTILLATION v2 (CONFIDENCE-WEIGHTED) ***")
    print(f"  Stage 1: {EPOCHS_STAGE1} epochs W2V + RSR")
    print(f"  Stage 2: {DISTILL_EPOCHS} epochs distillation")
    print(f"  Similarity threshold: {DISTILL_SIM_THRESHOLD}")
    print(f"  Confidence temperature: {DISTILL_CONFIDENCE_TEMP}")
    print(f"  K neighbors: {DISTILL_K_NEIGHBORS}, N targets: {DISTILL_N_TARGETS}")
    ...
    if output_path.exists():
        ...
        print(f"\nResuming. Completed seeds: {sorted(existing_seeds)}")
    else:
        existing_seeds = set()
        results = []
