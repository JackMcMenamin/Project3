...
Uses MULTIPLE datasets for RSR training (MEN + SimVerb + THINGS).
Evaluates on SimLex-999 with breakdown by training vocab membership.

Usage:
    python run_seeds_multi.py
    python run_seeds_multi.py --start_seed 1 --end_seed 20
"""
...
RSR_PAIRS_PER_STEP = 5000  # More pairs since we have more data
...
# ==============================================================================
# Multi-Dataset Loader (MEN + SimVerb + THINGS)
# ==============================================================================

def load_men_pairs(path: Path, word2idx: dict):
    """Load MEN dataset pairs."""
    pairs = []
    words = set()
...
def load_simverb_pairs(path: Path, word2idx: dict):
    """Load SimVerb-3500 dataset pairs."""
    pairs = []
    words = set()
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                score = float(parts[3])  # Column 4 is the score
            except:
                continue
...
def load_things_pairs(words_path: Path, sim_path: Path, word2idx: dict, max_pairs: int = 100000):
    """
    Load THINGS dataset - sample pairs from the similarity matrix.
    Returns a subset of pairs (since full matrix is 1.7M pairs).
    """
    pairs = []
    words = set()
    
    if not words_path.exists() or not sim_path.exists():
        print(f"[warn] THINGS not found: {words_path} or {sim_path}")
        return words, pairs
    ...
    if spose_sim is None:
        print("[warn] Could not find similarity matrix in THINGS .mat file")
        return words, pairs
    ...
    for ti, tw in enumerate(things_words):
        # Try original and space-removed versions
...
def load_all_datasets(word2idx: dict):
    """
    Load and combine all similarity datasets for RSR training.
    Returns:
        all_words: set of all unique words in combined datasets
        combined_pairs: numpy array of (idx1, idx2, normalized_score)
    """
    ...
    # Combine pairs and normalize scores to [0, 1]
    all_pairs = []
...
def sample_multi_pairs(num_pairs: int, pairs_array: np.ndarray):
    """Sample pairs from combined dataset for RSR training."""
...
def train_one_epoch_streaming(
    ...
    soft_rank_strength=2.0, pairs_array=None
):
    model.train()
    ...
    for b in pbar:
        ...
        if rsr_weight > 0.0 and pairs_array is not None and (b % rsr_every_n == 0):
            i_idx, j_idx, target_sim = sample_multi_pairs(rsr_pairs_per_step, pairs_array)
...
def simlex_spearman_by_train_count(model, simlex_df, word2idx, train_words, train_count=None):
    """
    Evaluate SimLex-999 Spearman correlation, optionally filtered by training vocab membership.
    
    Args:
        train_count: None = all pairs, 0 = neither word in training vocab, 
                     1 = exactly one word, 2 = both words
    """
    model.eval()
    ...
    for _, row in simlex_df.iterrows():
        ...
        # Count how many words are in training vocab
...
def run_one_seed(seed: int, word2idx, idx2word, idx_counts, train_words, pairs_array, simlex_df):
    ...
    # =========================================================================
    # Train RSR (with ramp-up, using combined datasets)
    # =========================================================================
    print(f"[seed={seed}] Training RSR (Multi-Dataset)...")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if warmup_batches > 0:
            train_one_epoch_streaming(
                ...
                rsr_weight=0.0, pairs_array=None,
            )
    ...
    # =========================================================================
    # Evaluate on SimLex-999 (by training vocab membership)
    # =========================================================================
    ...
    # 0 training words (neither word in training vocab)
...
def main():
    parser = argparse.ArgumentParser(description="Multi-seed Vanilla vs RSR (Multi-Dataset) training")
    ...
    parser.add_argument("--output", type=str, default="results/seed_results_multi.xlsx", help="Output Excel file")
    args = parser.parse_args()
    
    print(f"Device: {DEVICE}")
    print(f"Running seeds {args.start_seed} to {args.end_seed}")
    print(f"Output: {args.output}")
    print(f"RSR training data: MEN + SimVerb + THINGS (combined)")
    ...
    # =========================================================================
    # Load ALL datasets for RSR training
    # =========================================================================
    ...
    print(f"  0 train words:  {df['delta_0_train'].mean():.4f} ± {df['delta_0_train'].std():.4f}  (n={df['n_0_train'].iloc[0]})")
