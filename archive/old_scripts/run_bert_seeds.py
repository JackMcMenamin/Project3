"""
BERT RSR Multi-Seed Experiment

Runs BERT RSR fine-tuning across multiple seeds (1-10) to establish
statistical credibility. Results are saved to Excel and printed at the end.

Usage:
    python run_bert_seeds.py
"""
...
# ==============================================================================
# Helper Functions (copied from BERT.py)
# ==============================================================================
...
class BERTWordEmbeddings(nn.Module):
    ...
    def get_word_embedding(self, word: str) -> torch.Tensor:
        ...
        word_tokens_hidden = hidden_states[0, 1:-1, :]
        if word_tokens_hidden.shape[0] == 0:
            word_tokens_hidden = hidden_states[0, :1, :]
        
        word_embedding = word_tokens_hidden.mean(dim=0)
    ...
    def get_batch_embeddings(self, words: list, batch_size: int = 64) -> dict:
        ...
        for i in range(0, len(words), batch_size):
            ...
            tokens = self.tokenizer(batch_words, return_tensors="pt", padding=True,
...
def main():
    print("=" * 70)
    print("BERT RSR Multi-Seed Experiment")
    print(f"Seeds: {SEEDS}")
    ...
    excel_path = Path("results") / f"bert_seeds_{timestamp}.xlsx"
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("BERT RSR MULTI-SEED RESULTS SUMMARY")
    print("=" * 70)
